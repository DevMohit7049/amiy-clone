const Footer = () => {
  const footerSections = [
    ...
    },
    {
      ...
      links: [
        'Glow from Gut Duo',
        'Fibromyalgia Relief Duo',
        'PCOS Acne Relief Combo',
        'IBS Relief Gut Duo',
        'Inflammation Recovery Combo',
      ],
    },
    {
      title: 'Mini Products',
  ...
  return (
    <footer id="footer" style={{ backgroundColor: '#fcf7f1', color: '#6c6948' }}>
      {/* Main footer content */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-12 md:py-16">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-8">
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className="font-heading text-sm font-semibold uppercase text-text-header mb-4" style={{ color: '#424423' }}>
                {section.title}
              </h3>
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="text-xs md:text-sm transition-colors duration-200 hover:text-accent"
                      style={{ color: '#6c6948' }}
                    >
                      {link}
                    </a>
        ...
        {/* Divider */}
        <div className="my-10 md:my-14 border-t" style={{ borderColor: 'rgba(40, 42, 39, 0.3)' }} />

        {/* Social links section */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          {/* Logo */}
          <div>
            <span className="font-heading text-xl font-semibold" style={{ color: '#6c6948' }}>
              Amiy Naturals
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map((social) => (
              <a
                ...
                style={{
                  color: '#6c6948',
                  border: '1px solid rgba(108, 105, 72, 0.4)',
                }}
